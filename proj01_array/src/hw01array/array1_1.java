 ...
 * @author User
 */
public class array1_1 extends javax.swing.JFrame {
    ...
    static int display_idx = 0, insert_idx = 0, recordCount = 0;
    
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(array1_1.class.getName());
    ...
    public array1_1() {
        ...
        try {
            bgImage = new ImageIcon(getClass().getResource("/hw01array/ball.jpeg")).getImage();
        ...
        try {
            icon = new ImageIcon(getClass().getResource("/hw01array/icon2.png"))
            ...
            if (userSelection == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    if (recordCount > 0) {
                        ...
                        insertID(insert_idx);
                        showImage();
                        showDisplay(jLabel6, String.valueOf(display_idx), "Failed to update ID label in jPanel7: "); // ID
                        showDisplay(jLabel7, data.getName(display_idx), "Failed to update Name label in jPanel7: "); // Name
                        showDisplay(jLabel8, data.getCategory(display_idx), "Failed to update Category label in jPanel7: " ); // Category
                        showDisplay(jLabel9, data.getPrice(display_idx) + "$", "Failed to update Price label in jPanel7: " ); // Price
                        showDisplay(jLabel12, String.valueOf(insert_idx) + " (Default)", "Failed to update Total records label in jPanel7: "); // Records
                    } else {
                        JOptionPane.showMessageDialog(this,
    ...
    // ------------ DELETE PART ------------ 
    private Boolean deleteIndex(int deleteIndex){
         try {
            String imagePath = "src/images/" + data.getFileName(deleteIndex);
            foundImage = new ImageIcon(imagePath);

            // Return the icon if the name is found
            return true;
        } catch (Exception ex) {
            System.err.println("Failed to load image for item index " + deleteIndex + ": " + ex.getMessage());
            return false; // Return null if the image can't be loaded
        }
    }
    
    private int deleteName(String name) {
        for (int i = 0; i < recordCount; i++) {
            if (data.getName(i).equalsIgnoreCase(name)) {
                try {
                    String imagePath = "src/images/" + data.getFileName(i);
                    foundImage = new ImageIcon(imagePath);
                    ...
                    return -1; // Return null if the image can't be loaded
                }
        }
    }
        return -1; // Return null if the name is not found
    }
        
    private void displayDelName(int indexFound, String displayName){
        if (indexFound != -1) {
            ...
            // Confirmation dialog with YES/NO option
            int choice = JOptionPane.showConfirmDialog(
                this,
                "Name '" + data.getName(indexFound) + "' found in records."
                + "\nID: " + indexFound
                + "\nName: " + data.getName(indexFound)
                + "\nCategory: " + data.getCategory(indexFound)
                + "\nPrice: " + data.getPrice(indexFound) + "$"
            ...
            JOptionPane.showMessageDialog(this,
                displayName + " not found in records.",
    ...
    private void deletePriceRange() {
        int rowCount = tableModel.getRowCount();

        for (int i = 0; i < rowCount; i++) {
            try {
                int delIdx = (Integer) tableModel.getValueAt(i, 0);
                data.deleteRecord(delIdx);
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(
                    this,
                    "Error deleting record at row " + i + ": " + ex.getMessage(),
                    "Deletion Error",
                    JOptionPane.ERROR_MESSAGE
                );
            }
    ...
    private void resetInsert(){
        ...
        // 2. Reset jTextField1, jTextField2, jComboBox1, jLabel17
        jTextField1.setText("");
        jTextField2.setText("");
    ...
    private void showDisplay(javax.swing.JLabel label, String txt, String message){
        try {
            label.setText(txt);
        } catch (Exception ex) {
    ...
    private void showImage() {
        try {
            String imagePath = data.getFileName(display_idx);
    ...
    public int sortAndMakeScrollPane(boolean byName, boolean nameAsc,
        ...
        for (int K = 0; K < recordCount - 1; K++) {
            int PTR = 0;
            while (PTR < ((recordCount - 1) - K)) {
                int a = IDX[PTR];
                int b = IDX[PTR + 1];
                int result = 0;

                if (byName) {
                    result = data.getName(a).compareToIgnoreCase(data.getName(b));
                    if (!nameAsc) result = -result;
                }

                if (result == 0 && byCategory) {
                    result = data.getCategory(a).compareToIgnoreCase(data.getCategory(b));
                    if (!categoryAsc) result = -result;
                }

                if (result == 0 && byPrice) {
                    result = Float.compare(data.getPrice(a), data.getPrice(b));
        ...
        for (int i = 0; i < recordCount; i++) {
            int idx = IDX[i];
            Object[] row = {idx, data.getName(idx), data.getCategory(idx), data.getPrice(idx)};
    ...
    private int searchCategory(String category){
        ...
        int N = recordCount;
        while (I < N){
            if(data.getCategory(I).equals(category)){
                tableModel.addRow(new Object[] {
                    I,
                    data.getName(I),
                    data.getCategory(I),
    ...
    private void initComponents() {
        ...
        roundedPanel1 = new hw01array.RoundedPanel();
        ...
        roundedPanel3 = new hw01array.RoundedPanel();
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        ...
        int indexFound = searchName(name);
        ...
        if (indexFound != -1) {
            ...
            JOptionPane.showMessageDialog(this,
                ...
                + "\nPrice: " + data.getPrice(indexFound) + "$",
                "Search Result",
                JOptionPane.INFORMATION_MESSAGE, newIcon); // Pass the ImageIcon here
        } else {
            JOptionPane.showMessageDialog(this,
    ...
    private void jButton10ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton10ActionPerformed
        ...
        int indexFound = deleteName(deleteName);
        displayDelName(indexFound, "Name '" + deleteName + "' ");
    }//GEN-LAST:event_jButton10ActionPerformed

    private void jButton9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton9ActionPerformed
        // TODO add your handling code here:
        int del_idx = Integer.parseInt(jTextField4.getText());
        Boolean success = deleteIndex(del_idx);

        if(success){
            displayDelName(del_idx, "Index " + del_idx + " ");
        }
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        try {
            ...
            // Insert record
            Boolean isSuccess = data.insertRecord(name, type, price);
            ...
            JOptionPane.showMessageDialog(this, 
                "Price must be a valid number." + ex.getMessage(), 
                "Input Error", JOptionPane.ERROR_MESSAGE);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this,
    ...
    private void jButton13ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton13ActionPerformed
        // TODO add your handling code here:
        showImage();
        showDisplay(jLabel6, String.valueOf(display_idx), "Failed to update ID label: "); // Show id
        showDisplay(jLabel7, data.getName(display_idx), "Failed to update Name label: "); // Show name
        showDisplay(jLabel8, data.getCategory(display_idx), "Failed to update Category label: " ); // Show category
        showDisplay(jLabel9, data.getPrice(display_idx) + "$", "Failed to update Price label: " ); // Show price
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if(display_idx >= data.getLenFile()) display_idx = data.getLenFile() - 1;
    ...
    private void jButton14ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton14ActionPerformed
        ...
        try {
            indexFound = Integer.parseInt(jTextField6.getText());
        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(this, "Index must be a valid integer.");
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Unexpected error: " + ex.getMessage());
        }
        
        if (indexFound >= 0 && indexFound < insert_idx) {
            try {
                String imagePath = "src/images/" + data.getFileName(indexFound);
                foundImage = new ImageIcon(imagePath);
                Image scaledImg = foundImage.getImage().getScaledInstance(257, 231, Image.SCALE_SMOOTH);
                ImageIcon newIcon = new ImageIcon(scaledImg);
                
                JOptionPane.showMessageDialog(this,
                "ID '" + indexFound + "'  found in records."
                + "\nID: " + indexFound
                + "\nName: " + data.getName(indexFound)
                + "\nCategory: " + data.getCategory(indexFound)
                + "\nPrice: " + data.getPrice(indexFound) + "$",
                ...
                JOptionPane.showMessageDialog(this,
                "ID '" + indexFound + "' found in records, but its image not.",
                "Search Result",
                JOptionPane.WARNING_MESSAGE);
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Unexpected error: " + ex.getMessage());
            ...
            JOptionPane.showMessageDialog(this,
                "ID '" + indexFound + "' not found in records.",
                "Search Result",
                JOptionPane.WARNING_MESSAGE);
        }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> new array1_1().setVisible(true));
    }
    ...
    private hw01array.RoundedPanel roundedPanel1;
    private hw01array.RoundedPanel roundedPanel3;
