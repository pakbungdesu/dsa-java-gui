 ...
 * @author User
 */
public class hw_stack1 extends javax.swing.JFrame {
    
    static int SIZE;
    int TOP_A = -1, TOP_B = -1, TOP_C = -1;
    int MAXSTK_A, MAXSTK_B, MAXSTK_C;
    int ITEM_A, ITEM_B, ITEM_C;
    int[] STACKA, STACKB, STACKC;

     public boolean isEmpty(String whichStack) {
        switch (whichStack) {
            case "A": return TOP_A == -1;
            case "B": return TOP_B == -1;
            case "C": return TOP_C == -1;
        }
        return true;
    }
    ...
    public boolean isFull(String whichStack) {
        switch (whichStack) {
            case "A": return TOP_A == MAXSTK_A;
            case "B": return TOP_B == MAXSTK_B;
            case "C": return TOP_C == MAXSTK_C;
        }
        return true;
    }
        ...
        try {
            switch (whichStack) {
                case "A":
                    TOP_A++;
                    STACKA[TOP_A] = ITEM_A;
                    jTable4.setValueAt(STACKA[TOP_A], MAXSTK_A - TOP_A, 0);
                    return true;

                case "B":
                    TOP_B++;
                    STACKB[TOP_B] = ITEM_B;
                    jTable5.setValueAt(STACKB[TOP_B], MAXSTK_B - TOP_B, 0);
                    return true;

                case "C":
                    TOP_C++;
                    STACKC[TOP_C] = ITEM_C;
                    jTable6.setValueAt(STACKC[TOP_C], MAXSTK_C - TOP_C, 0);
                    return true;
            ...
            JOptionPane.showMessageDialog(this,
                "Stack overflow or invalid index access!",
                "Array Error",
                JOptionPane.ERROR_MESSAGE);
            return false;
    ...
    public boolean Pop(String whichStack) {
        switch (whichStack) {
            case "A":
                ITEM_A = STACKA[TOP_A];
                jTable4.setValueAt(null, MAXSTK_A - TOP_A, 0);
                TOP_A--;
                return true;

            case "B":
                ITEM_B = STACKB[TOP_B];
                jTable5.setValueAt(null, MAXSTK_B - TOP_B, 0);
                TOP_B--;
                return true;

            case "C":
                ITEM_C = STACKC[TOP_C];
                jTable6.setValueAt(null, MAXSTK_C - TOP_C, 0);
                TOP_C--;
                return true;
    ...
    public void pushAndCheck(String whichStack) {
        ...
        switch (whichStack) {
            case "A":
                currentItem = ITEM_A;
                if (TOP_A >= 0) topItem = STACKA[TOP_A];
                break;
            case "B":
                currentItem = ITEM_B;
                if (TOP_B >= 0) topItem = STACKB[TOP_B];
                break;
            case "C":
                currentItem = ITEM_C;
                if (TOP_C >= 0) topItem = STACKC[TOP_C];
                break;
        }

        // --- Validate: new item must be LESS than top ---
        if (topItem != -1 && currentItem > topItem) {
            JOptionPane.showMessageDialog(this,
                    "ITEM (" + currentItem + ") is greater than top of stack (" + topItem + ").\n"
        ...
        boolean isSuccess = Push(whichStack);
        if (isSuccess) {
            JOptionPane.showMessageDialog(this,
    ...
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(hw_stack1.class.getName());

    /**
     * Creates new form hw_stack1
     */
    public hw_stack1(int size) {
    ...
    private void jButton12ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton12ActionPerformed
        ...
        ITEM_B = STACKA[TOP_A];  // Peek top of A
        if (isFull("B")) {
            JOptionPane.showMessageDialog(this,
    ...
    private void jButton14ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton14ActionPerformed
        ...
        ITEM_C = STACKB[TOP_B];  // Peek top of B
        if (isFull("C")) {
            JOptionPane.showMessageDialog(this,
    ...
    private void jButton11ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        ITEM_A = STACKB[TOP_B];  // Peek top of B
        if (isFull("A")) {
            JOptionPane.showMessageDialog(this,
    ...
    private void jButton13ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton13ActionPerformed
        ...
        ITEM_B = STACKC[TOP_C];  // Peek top of C
        if (isFull("B")) {
            JOptionPane.showMessageDialog(this,
                "Stack B Overflow! Cannot move item.",
                "Stack Overflow",
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> new hw_stack1(SIZE).setVisible(true));
    }
